#ifndef YM_VWAP_LOGIC_H
#define YM_VWAP_LOGIC_H

enum YM_VWAP_ENTRY_LEVEL
{
    YM_VWAP_LOGIC_ENTRY_LEVEL_ZERO = 0,
    YM_VWAP_LOGIC_ENTRY_LEVEL_ONE,
    YM_VWAP_LOGIC_ENTRY_LEVEL_TWO,
    YM_VWAP_LOGIC_ENTRY_LEVEL_THREE,
    YM_VWAP_LOGIC_ENTRY_LEVEL_FOUR,
    YM_VWAP_LOGIC_ENTRY_LEVEL_FIVE
};

enum YM_VWAP_LOGIC_STATUS
{
    YM_VWAP_LOGIC_ENTRY_WAIT = 0,
    YM_VWAP_LOGIC_LONG,
    YM_VWAP_LOGIC_SHORT,
    YM_VWAP_LOGIC_CLOSED
};

class YMVWAPLogic : public TradeLogic
{
    public:

        YMVWAPLogic( const System& system )
        {
            last_timebar_timestamp = 0;
            VALUE = VOLUME = VWAP = 0.0;

            multiplier = 5.0;
            status = YM_VWAP_LOGIC_ENTRY_WAIT;
            curr_entry_level = YM_VWAP_LOGIC_ENTRY_LEVEL_ZERO;

            price_offset[ 0 ] = 60;
            price_offset[ 1 ] = 90;
            price_offset[ 2 ] = 150;
            price_offset[ 3 ] = 5000; //24;
            price_offset[ 4 ] = 5000; //36;
        ...
        void addExecutionInternal( )
        {
            ...
            {
                status = YM_VWAP_LOGIC_LONG;
            }
            else
            {
                status = YM_VWAP_LOGIC_SHORT;
            }
        }
        ...
        bool evaluate( System& system, std::vector< TradeInstruction* >& trade_instruction_vec )
        {
            ...
            if( last_timebar_timestamp != system.five_minute_timebar->timestamp )
            {
                ...
                if( status == YM_VWAP_LOGIC_ENTRY_WAIT )
                {
                    system.log->info( " STATUS: YM_VWAP_LOGIC_ENTRY_WAIT" );
                }
                else if( status == YM_VWAP_LOGIC_LONG )
                {
                    system.log->info( " STATUS: YM_VWAP_LOGIC_LONG" );
                }
                else if( status == YM_VWAP_LOGIC_SHORT )
                {
                    system.log->info( " STATUS: YM_VWAP_LOGIC_SHORT" );
                }

                if( curr_entry_level == YM_VWAP_LOGIC_ENTRY_LEVEL_FIVE )
                {
                    system.log->info( " CURR_ENTRY_LEVEL: YM_VWAP_LOGIC_ENTRY_LEVEL_FIVE" );
                }
                else if( curr_entry_level == YM_VWAP_LOGIC_ENTRY_LEVEL_FOUR )
                {
                    system.log->info( " CURR_ENTRY_LEVEL: YM_VWAP_LOGIC_ENTRY_LEVEL_FOUR" );
                }
                else if( curr_entry_level == YM_VWAP_LOGIC_ENTRY_LEVEL_THREE )
                {
                    system.log->info( " CURR_ENTRY_LEVEL: YM_VWAP_LOGIC_ENTRY_LEVEL_THREE" );
                }
                else if( curr_entry_level == YM_VWAP_LOGIC_ENTRY_LEVEL_TWO )
                {
                    system.log->info( " CURR_ENTRY_LEVEL: YM_VWAP_LOGIC_ENTRY_LEVEL_TWO" );
                }
                else if( curr_entry_level == YM_VWAP_LOGIC_ENTRY_LEVEL_ONE )
                {
                    system.log->info( " CURR_ENTRY_LEVEL: YM_VWAP_LOGIC_ENTRY_LEVEL_ONE" );
                }
                else if( curr_entry_level == YM_VWAP_LOGIC_ENTRY_LEVEL_ZERO )
                {
                    system.log->info( " CURR_ENTRY_LEVEL: YM_VWAP_LOGIC_ENTRY_LEVEL_ZERO" );
                }
                    ...
                    system.five_minute_timebar->timestamp < stop_entry_tm )
                {
                    ...
                    if( best_ask > VWAP )
                    {
                        ...
                        if( curr_entry_level == YM_VWAP_LOGIC_ENTRY_LEVEL_ZERO )
                        {
                            if( best_ask > entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_ZERO ] )
                            {
                                entry_qty = position_multiplier[ YM_VWAP_LOGIC_ENTRY_LEVEL_ZERO ];
                                exit_level_one[ 0 ] = false;
                                curr_entry_level = YM_VWAP_LOGIC_ENTRY_LEVEL_ONE;

                                system.log->info( "  ENTRY_LEVEL_ZERO: %f > %f, SELL %ld",
                                    best_ask, entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_ZERO ], entry_qty );
                            }
                            else
                            {
                                system.log->info( "  FAIL ENTRY_LEVEL_ZERO: %f <= %f",
                                    best_ask, entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_ZERO ] );
                            }
                        }
                        else if( curr_entry_level == YM_VWAP_LOGIC_ENTRY_LEVEL_ONE )
                        {
                            if( best_ask > entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_ONE ] )
                            {
                                entry_qty = position_multiplier[ YM_VWAP_LOGIC_ENTRY_LEVEL_ONE ];
                                exit_level_two[ 0 ] = exit_level_two[ 1 ] = false;
                                curr_entry_level = YM_VWAP_LOGIC_ENTRY_LEVEL_TWO;

                                system.log->info( "  ENTRY_LEVEL_ONE: %f > %f, SELL %ld",
                                    best_ask, entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_ONE ], entry_qty );
                            }
                            else
                            {
                                system.log->info( "  FAIL ENTRY_LEVEL_ONE: %f <= %f",
                                    best_ask, entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_ONE ] );
                            }
                        }
                        else if( curr_entry_level == YM_VWAP_LOGIC_ENTRY_LEVEL_TWO )
                        {
                            if( best_ask > entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_TWO ] )
                            {
                                entry_qty = position_multiplier[ YM_VWAP_LOGIC_ENTRY_LEVEL_TWO ];
                                exit_level_three[ 0 ] = exit_level_three[ 1 ] = exit_level_three[ 2 ] = false;
                                curr_entry_level = YM_VWAP_LOGIC_ENTRY_LEVEL_THREE;

                                system.log->info( "  ENTRY_LEVEL_TWO: %f > %f, SELL %ld",
                                    best_ask, entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_TWO ], entry_qty );
                            }
                            else
                            {
                                system.log->info( "  FAIL ENTRY_LEVEL_TWO: %f <= %f",
                                    best_ask, entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_TWO ] );
                            }
                        }
                    }
                    else if( best_bid < VWAP )
                    {
                        ...
                        if( curr_entry_level == YM_VWAP_LOGIC_ENTRY_LEVEL_ZERO )
                        {
                            if( best_bid < entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_ZERO ] )
                            {
                                entry_qty = position_multiplier[ YM_VWAP_LOGIC_ENTRY_LEVEL_ZERO ];
                                exit_level_one[ 0 ] = false;
                                curr_entry_level = YM_VWAP_LOGIC_ENTRY_LEVEL_ONE;
                                ...
                                system.log->info( "  ENTRY_LEVEL_ZERO: %f < %f, BUY %ld",
                                    best_bid, entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_ZERO ], entry_qty );
                            }
                            else
                            {
                                system.log->info( "  FAIL ENTRY_LEVEL_ZERO: %f >= %f",
                                    best_bid, entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_ZERO ] );
                            }
                        }
                        else if( curr_entry_level == YM_VWAP_LOGIC_ENTRY_LEVEL_ONE )
                        {
                            if( best_bid < entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_ONE ] )
                            {
                                entry_qty = position_multiplier[ YM_VWAP_LOGIC_ENTRY_LEVEL_ONE ];
                                exit_level_two[ 0 ] = exit_level_two[ 1 ] = false;
                                curr_entry_level = YM_VWAP_LOGIC_ENTRY_LEVEL_TWO;
                                ...
                                system.log->info( "  ENTRY_LEVEL_ONE: %f < %f, BUY %ld",
                                    best_bid, entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_ONE ], entry_qty );
                            }
                            else
                            {
                                system.log->info( "  FAIL ENTRY_LEVEL_ONE: %f >= %f",
                                    best_bid, entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_ONE ] );
                            }
                        }
                        else if( curr_entry_level == YM_VWAP_LOGIC_ENTRY_LEVEL_TWO )
                        {
                            if( best_bid < entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_TWO ] )
                            {
                                entry_qty = position_multiplier[ YM_VWAP_LOGIC_ENTRY_LEVEL_TWO ];
                                exit_level_three[ 0 ] = exit_level_three[ 1 ] = exit_level_three[ 2 ] = false;
                                curr_entry_level = YM_VWAP_LOGIC_ENTRY_LEVEL_THREE;
                                ...
                                system.log->info( "  ENTRY_LEVEL_TWO: %f < %f, BUY %ld",
                                    best_bid, entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_TWO ], entry_qty );
                            }
                            else
                            {
                                system.log->info( "  FAIL ENTRY_LEVEL_TWO: %f >= %f",
                                    best_bid, entry_level[ YM_VWAP_LOGIC_ENTRY_LEVEL_TWO ] );
                            }
                        }
                    }
            ...
            if( status == YM_VWAP_LOGIC_LONG )
            {
                exit_prc = best_bid;
                exit_side = TRADE_INSTRUCTION_SIDE_SHORT;
                const double system_pl = getSystemPL( best_ask, best_bid );

                if( curr_entry_level >= YM_VWAP_LOGIC_ENTRY_LEVEL_THREE )
                {
                    exit_level[ 0 ] = VWAP - 115;
                    exit_level[ 1 ] = VWAP - 95;
                    exit_level[ 2 ] = VWAP - 62.5;
                ...
                if( curr_entry_level >= YM_VWAP_LOGIC_ENTRY_LEVEL_TWO )
                {
                    exit_level[ 0 ] = VWAP - 57.5;
                    exit_level[ 1 ] = VWAP - 35;
                ...
                if( curr_entry_level >= YM_VWAP_LOGIC_ENTRY_LEVEL_ONE )
                {
                    exit_level[ 0 ] = VWAP - 25;
                ...
                {
                    system.log->warn( "YM_VWAP_LOGIC_LONG_EXIT: DAILY_LOSS_LIMIT %f @ %ld", system_pl, system.now );
            ...
            else if( status == YM_VWAP_LOGIC_SHORT )
            {
                ...
                if( system_pl < DAILY_LOSS_LIMIT )
                {
                    system.log->warn( "YM_VWAP_LOGIC_SHORT_EXIT: DAILY_LOSS_LIMIT %f @ %ld", system_pl, system.now );
    ...
    private:
        ...
        YM_VWAP_LOGIC_STATUS status;
        YM_VWAP_ENTRY_LEVEL curr_entry_level;

};

#endif // YM_VWAP_LOGIC_H
