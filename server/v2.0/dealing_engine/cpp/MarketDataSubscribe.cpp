...
const void dealing::RequestHandler::marketDataSubscribe(
    ...
    quantmodel::DatabaseEngineRequest& database_msg )
{
    ...
    for( int i=0; i<request_msg.instrument_data_size(); ++i )
    {
        fix_request.Clear();
        fix_response.Clear();

        const quantmodel::InstrumentData& instrument_data = request_msg.instrument_data( i );

        //
        //  Find this instrument in the instrument_map
        //

        if( ( instrument_iter = instrument_map.find( instrument_data.instrument_id() ) ) != instrument_map.end() )
        {
            instrument_iter->second->copyTo( fix_request.mutable_instrument_data() );
