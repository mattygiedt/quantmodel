...
void processTradeInstructionVector( System& system, std::vector< TradeInstruction* >& trade_instruction_vec )
{
    if( likely( trade_instruction_vec.size() > 0 ) )
    {
             ...
             ++trade_instruction_iter )
        {
            ...
            handleTradeInstruction( instruction, system );
            delete instruction;
        }
...
int main( int argc, char *argv[] )
{
    ...
    while( ( rc = zmq::poll( items, 3, -1 ) ) >= 0 )
    {
        if( items[ 0 ].revents & ZMQ_POLLIN )
        {
            ...
            if( dealing_broadcast_msg.type() == quantmodel::DealingEngineBroadcast::CREATE_EXECUTION )
            {
                for( int i=0; i<dealing_broadcast_msg.order_size(); ++i )
                {
                    const quantmodel::OrderMessage& order_message = dealing_broadcast_msg.order( i );
                    const quantmodel::OrderData& order_data = order_message.order_data( );

                    if( order_data.order_status() == "5" && // FILLED
                        order_data.investment_system_id() == system.investment_system_id &&
                        order_data.instrument_id() == system.instrument_id )
                    {
                        bool found_order = false;
                        std::vector< OrderExecution* >::const_iterator order_execution_iter;

                        //
                        //  We *should* only see this message once per order...
                        //

                        for( order_execution_iter = system.order_execution_vec.begin();
                             order_execution_iter != system.order_execution_vec.end();
                             ++order_execution_iter )
                        {
                            if( order_data.order_id() == (*order_execution_iter)->order_id )
                            {
                                system.log->error( "RECEIVED DUPLICATE CREATE_EXECUTION FOR FILLED ORDER: %s", order_data.order_id().c_str() );
                                found_order = true;
                                break;
                            }
                        }
                        ...
                        if( !found_order )
                        {
                            ...
                            if( order_data.side_code() == "1" ) // BUY
                            {
                                exec = new OrderExecution(
                                    system.now,
                                    system.five_minute_timebar->timestamp,
                                    atof( order_data.average_prc().c_str() ),
                                    atoi( order_data.executed_qty().c_str() ),
                                    SIDE_CODE_BUY,
                                    order_data.order_id() );
                            }
                            else if( order_data.side_code() == "2" ) // SELL
                            {
                                exec = new OrderExecution(
                                    system.now,
                                    system.five_minute_timebar->timestamp,
                            ...
                            {
                                system.log->error( "UNKNOWN SIDE_CODE '%s' FOR ORDER: %s",
                                    order_data.side_code().c_str(),
                                    order_data.order_id().c_str() );
                            }

                            if( exec != NULL )
                            {
                                system.order_execution_vec.push_back( exec );

                                //
                                //  Broadcast the order execution
                                //

                                broadcast_msg.Clear();
                                system.fillOrderExecutionData( exec, broadcast_msg );
