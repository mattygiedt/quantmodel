#ifndef ES_VWAP_LOGIC_H
#define ES_VWAP_LOGIC_H

enum ES_VWAP_ENTRY_LEVEL
{
    ES_VWAP_LOGIC_ENTRY_LEVEL_ZERO = 0,
    ES_VWAP_LOGIC_ENTRY_LEVEL_ONE,
    ES_VWAP_LOGIC_ENTRY_LEVEL_TWO,
    ES_VWAP_LOGIC_ENTRY_LEVEL_THREE,
    ES_VWAP_LOGIC_ENTRY_LEVEL_FOUR,
    ES_VWAP_LOGIC_ENTRY_LEVEL_FIVE
};

enum ES_VWAP_LOGIC_STATUS
{
    ES_VWAP_LOGIC_ENTRY_WAIT = 0,
    ES_VWAP_LOGIC_LONG,
    ES_VWAP_LOGIC_SHORT,
    ES_VWAP_LOGIC_CLOSED
};

class ESVWAPLogic : public TradeLogic
{
    public:

        ESVWAPLogic( const System& system )
        {
            last_timebar_timestamp = 0;
            VALUE = VOLUME = VWAP = 0.0;

            multiplier = 50.0;
            status = ES_VWAP_LOGIC_ENTRY_WAIT;
            curr_entry_level = ES_VWAP_LOGIC_ENTRY_LEVEL_ZERO;

            price_offset[ 0 ] = 6.25;
            price_offset[ 1 ] = 9;
            price_offset[ 2 ] = 15;
            price_offset[ 3 ] = 500; //24;
            price_offset[ 4 ] = 500; //36;
        ...
        void addExecutionInternal( )
        {
            ...
            {
                status = ES_VWAP_LOGIC_LONG;
            }
            else
            {
                status = ES_VWAP_LOGIC_SHORT;
            }
        }
        ...
        bool evaluate( System& system, std::vector< TradeInstruction* >& trade_instruction_vec )
        {
            ...
            if( last_timebar_timestamp != system.five_minute_timebar->timestamp )
            {
                ...
                if( status == ES_VWAP_LOGIC_ENTRY_WAIT )
                {
                    system.log->info( " STATUS: ES_VWAP_LOGIC_ENTRY_WAIT" );
                }
                else if( status == ES_VWAP_LOGIC_LONG )
                {
                    system.log->info( " STATUS: ES_VWAP_LOGIC_LONG" );
                }
                else if( status == ES_VWAP_LOGIC_SHORT )
                {
                    system.log->info( " STATUS: ES_VWAP_LOGIC_SHORT" );
                }

                if( curr_entry_level == ES_VWAP_LOGIC_ENTRY_LEVEL_FIVE )
                {
                    system.log->info( " CURR_ENTRY_LEVEL: ES_VWAP_LOGIC_ENTRY_LEVEL_FIVE" );
                }
                else if( curr_entry_level == ES_VWAP_LOGIC_ENTRY_LEVEL_FOUR )
                {
                    system.log->info( " CURR_ENTRY_LEVEL: ES_VWAP_LOGIC_ENTRY_LEVEL_FOUR" );
                }
                else if( curr_entry_level == ES_VWAP_LOGIC_ENTRY_LEVEL_THREE )
                {
                    system.log->info( " CURR_ENTRY_LEVEL: ES_VWAP_LOGIC_ENTRY_LEVEL_THREE" );
                }
                else if( curr_entry_level == ES_VWAP_LOGIC_ENTRY_LEVEL_TWO )
                {
                    system.log->info( " CURR_ENTRY_LEVEL: ES_VWAP_LOGIC_ENTRY_LEVEL_TWO" );
                }
                else if( curr_entry_level == ES_VWAP_LOGIC_ENTRY_LEVEL_ONE )
                {
                    system.log->info( " CURR_ENTRY_LEVEL: ES_VWAP_LOGIC_ENTRY_LEVEL_ONE" );
                }
                else if( curr_entry_level == ES_VWAP_LOGIC_ENTRY_LEVEL_ZERO )
                {
                    system.log->info( " CURR_ENTRY_LEVEL: ES_VWAP_LOGIC_ENTRY_LEVEL_ZERO" );
                }
                    ...
                    system.five_minute_timebar->timestamp < stop_entry_tm )
                {
                    ...
                    if( best_ask > VWAP )
                    {
                        ...
                        if( curr_entry_level == ES_VWAP_LOGIC_ENTRY_LEVEL_ZERO )
                        {
                            if( best_ask > entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_ZERO ] )
                            {
                                entry_qty = position_multiplier[ ES_VWAP_LOGIC_ENTRY_LEVEL_ZERO ];
                                exit_level_one[ 0 ] = false;
                                curr_entry_level = ES_VWAP_LOGIC_ENTRY_LEVEL_ONE;

                                system.log->info( "  ENTRY_LEVEL_ZERO: %f > %f, SELL %ld",
                                    best_ask, entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_ZERO ], entry_qty );
                            }
                            else
                            {
                                system.log->info( "  FAIL ENTRY_LEVEL_ZERO: %f <= %f",
                                    best_ask, entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_ZERO ] );
                            }
                        }
                        else if( curr_entry_level == ES_VWAP_LOGIC_ENTRY_LEVEL_ONE )
                        {
                            if( best_ask > entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_ONE ] )
                            {
                                entry_qty = position_multiplier[ ES_VWAP_LOGIC_ENTRY_LEVEL_ONE ];
                                exit_level_two[ 0 ] = exit_level_two[ 1 ] = false;
                                curr_entry_level = ES_VWAP_LOGIC_ENTRY_LEVEL_TWO;

                                system.log->info( "  ENTRY_LEVEL_ONE: %f > %f, SELL %ld",
                                    best_ask, entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_ONE ], entry_qty );
                            }
                            else
                            {
                                system.log->info( "  FAIL ENTRY_LEVEL_ONE: %f <= %f",
                                    best_ask, entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_ONE ] );
                            }
                        }
                        else if( curr_entry_level == ES_VWAP_LOGIC_ENTRY_LEVEL_TWO )
                        {
                            if( best_ask > entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_TWO ] )
                            {
                                entry_qty = position_multiplier[ ES_VWAP_LOGIC_ENTRY_LEVEL_TWO ];
                                exit_level_three[ 0 ] = exit_level_three[ 1 ] = exit_level_three[ 2 ] = false;
                                curr_entry_level = ES_VWAP_LOGIC_ENTRY_LEVEL_THREE;

                                system.log->info( "  ENTRY_LEVEL_TWO: %f > %f, SELL %ld",
                                    best_ask, entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_TWO ], entry_qty );
                            }
                            else
                            {
                                system.log->info( "  FAIL ENTRY_LEVEL_TWO: %f <= %f",
                                    best_ask, entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_TWO ] );
                            }
                        }
                    }
                    else if( best_bid < VWAP )
                    {
                        ...
                        if( curr_entry_level == ES_VWAP_LOGIC_ENTRY_LEVEL_ZERO )
                        {
                            if( best_bid < entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_ZERO ] )
                            {
                                entry_qty = position_multiplier[ ES_VWAP_LOGIC_ENTRY_LEVEL_ZERO ];
                                exit_level_one[ 0 ] = false;
                                curr_entry_level = ES_VWAP_LOGIC_ENTRY_LEVEL_ONE;
                                ...
                                system.log->info( "  ENTRY_LEVEL_ZERO: %f < %f, BUY %ld",
                                    best_bid, entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_ZERO ], entry_qty );
                            }
                            else
                            {
                                system.log->info( "  FAIL ENTRY_LEVEL_ZERO: %f >= %f",
                                    best_bid, entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_ZERO ] );
                            }
                        }
                        else if( curr_entry_level == ES_VWAP_LOGIC_ENTRY_LEVEL_ONE )
                        {
                            if( best_bid < entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_ONE ] )
                            {
                                entry_qty = position_multiplier[ ES_VWAP_LOGIC_ENTRY_LEVEL_ONE ];
                                exit_level_two[ 0 ] = exit_level_two[ 1 ] = false;
                                curr_entry_level = ES_VWAP_LOGIC_ENTRY_LEVEL_TWO;
                                ...
                                system.log->info( "  ENTRY_LEVEL_ONE: %f < %f, BUY %ld",
                                    best_bid, entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_ONE ], entry_qty );
                            }
                            else
                            {
                                system.log->info( "  FAIL ENTRY_LEVEL_ONE: %f >= %f",
                                    best_bid, entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_ONE ] );
                            }
                        }
                        else if( curr_entry_level == ES_VWAP_LOGIC_ENTRY_LEVEL_TWO )
                        {
                            if( best_bid < entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_TWO ] )
                            {
                                entry_qty = position_multiplier[ ES_VWAP_LOGIC_ENTRY_LEVEL_TWO ];
                                exit_level_three[ 0 ] = exit_level_three[ 1 ] = exit_level_three[ 2 ] = false;
                                curr_entry_level = ES_VWAP_LOGIC_ENTRY_LEVEL_THREE;
                                ...
                                system.log->info( "  ENTRY_LEVEL_TWO: %f < %f, BUY %ld",
                                    best_bid, entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_TWO ], entry_qty );
                            }
                            else
                            {
                                system.log->info( "  FAIL ENTRY_LEVEL_TWO: %f >= %f",
                                    best_bid, entry_level[ ES_VWAP_LOGIC_ENTRY_LEVEL_TWO ] );
                            }
                        }
                    }
            ...
            if( status == ES_VWAP_LOGIC_LONG )
            {
                ...
                if( curr_entry_level >= ES_VWAP_LOGIC_ENTRY_LEVEL_THREE )
                {
                    exit_level[ 0 ] = VWAP - 11.5;
                    exit_level[ 1 ] = VWAP - 9.5;
                    exit_level[ 2 ] = VWAP - 6.25;
                ...
                if( curr_entry_level >= ES_VWAP_LOGIC_ENTRY_LEVEL_TWO )
                {
                    exit_level[ 0 ] = VWAP - 5.75;
                    exit_level[ 1 ] = VWAP - 3.5;
                ...
                if( curr_entry_level >= ES_VWAP_LOGIC_ENTRY_LEVEL_ONE )
                {
                    exit_level[ 0 ] = VWAP - 2.5;
                ...
                {
                    system.log->warn( "ES_VWAP_LOGIC_LONG_EXIT: DAILY_LOSS_LIMIT %f @ %ld", system_pl, system.now );
            ...
            else if( status == ES_VWAP_LOGIC_SHORT )
            {
                ...
                if( system_pl < DAILY_LOSS_LIMIT )
                {
                    system.log->warn( "ES_VWAP_LOGIC_SHORT_EXIT: DAILY_LOSS_LIMIT %f @ %ld", system_pl, system.now );
    ...
    private:
        ...
        ES_VWAP_LOGIC_STATUS status;
        ES_VWAP_ENTRY_LEVEL curr_entry_level;

};

#endif // ES_VWAP_LOGIC_H
